export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && url.pathname === "/webhook/apify") {
      try {
        ...
        for (const lead of leads) {
          ...
          let dmTekst = "Hoi! Gefeliciteerd met je start. Heb je toevallig al nagedacht over een website?";
          ...
          try {
            ...
            dmTekst =
              geminiData.candidates?.[0]?.content?.parts?.[0]?.text || dmTekst;
          } catch (geminiErr) {
            // Gemini mislukt → ga door met fallback tekst
            console.error("Gemini fout:", geminiErr);
          }

          // 4. Sla op in D1
          const leadId = crypto.randomUUID();
          await env.DB.prepare(
            "INSERT INTO instagram_leads (id, username, full_name, biography, ai_message, status) VALUES (?, ?, ?, ?, ?, ?)"
          )
            .bind(leadId, lead.username, lead.fullName || "", lead.biography || "", dmTekst, "nieuw")
            .run();
            ...
            await fetch(
              `https://api.telegram.org/bot${env.TELEGRAM_BOT_TOKEN}/sendMessage`,
              {
                ...
                body: JSON.stringify({
                  ...
                  text: telegramBericht,
                  parse_mode: "Markdown",
                }),
              }
            );
          } catch (telegramErr) {
            console.error("Telegram fout:", telegramErr);
          }
        }
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // ROUTE 2: DASHBOARD API — Leads ophalen (nieuw!)
    // ──────────────────────────────────────────────────────────────────────────
    if (request.method === "GET" && url.pathname === "/api/admin/leads") {
      ...
      try {
        ...
        return new Response(
          JSON.stringify({ error: "Database fout: " + err.message }),
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // ROUTE 3: DASHBOARD API — Status updaten
    // ──────────────────────────────────────────────────────────────────────────
    if (request.method === "PATCH" && url.pathname.startsWith("/api/admin/leads/")) {
      ...
      try {
        ...
        if (!toegestaan.includes(status)) {
          return new Response(
            JSON.stringify({ error: `Ongeldige status "${status}". Toegestaan: ${toegestaan.join(", ")}` }),
            { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
          );
        }

        await env.DB.prepare(
          "UPDATE instagram_leads SET status = ? WHERE id = ?"
